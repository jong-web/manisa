				...
				<c:otherwise>
					<c:forEach var="cart" items="${ cartList }">
						<tr id="cart-item-${cart.cartID}">
							<td class="td1"><input type="checkbox" name="choice" checked onchange="updateTotalAmount()"></td>
							<td class="td2"><img src="images/${ cart.imageUrl }" alt="${ cart.bookName }"></td>
							<td class="td3">
								<span>소득공제<br></span>
								${ cart.bookName }<br>
								<i>(10% 할인)&nbsp;&nbsp;<b>P </b>900원</i>
							</td>
							<td class="td4"><input type="number" value="${ cart.num }" min="1" onchange="updateNum(${cart.cartID},this.value)"></td>
							<td class="td5" id="item-price-${cart.cartID}">
								<fmt:formatNumber value="${ cart.price * cart.num }" pattern="#,###"/>원
							</td>
							<td class="td6">
								<p>내일 아침 7시 전</p>
								<span>16시 전에 도착예정</span><br>
								<button type="button">아침배송</button>
							</td>
							<td class="td7">
								<div class="btn">
									<button type="button" onclick="orderItem(${cart.cartID})">주문하기</button>
									<button type="button" onclick="location.href='bookList.do'">나중에 주문</button>
	...
	<script>
		...
		function updateTotalAmount() {
			...
			$("input[name='choice']:checked").each(function(){
				...
				let price = parseInt(row.find("td.td5").text().replace("원", "").split(',').join(""));
				// totalAmount변수에 price(금액) 누적합
				totalAmount += price;
			});
			// 총금액을 아이디가 total-amount인 영역에 표시
			// .toLocaleString() : 숫자에 천단위 구분기호를 붙여서 표시
			...
			$.ajax({
				...
				success : function(response){
					...
					if(response.emptyCart){
						$("#cart-table").append("<tr id='emptyCart'><td colspan='7' class='center-box'>장바구니가 비었습니다.</td></tr>");
					}
				},
				error : function(){
					alert("도서 삭제 중 오류 발생!");
