		...
		<table>
			...
			<tr>
				<td colspan="2" class="center-btn">
					<!-- 로그인하면 장바구니에 추가 버튼 보이고, 로그인 안하면 안보임 -->
					<c:choose>
						<c:when test="${ not empty sessionScope.userID && sessionScope.userID == dto.userID }">
							<button type="button" onclick="location.href='pass.do?mode=edit&bookID=${ dto.bookID }'">수정하기</button>
							<button type="button" onclick="location.href='pass.do?mode=delete&bookID=${ dto.bookID }'">삭제하기</button>
						</c:when>
						<c:otherwise>
							<button type="button" onclick="location.href='login.do'">로그인 후 수정, 삭제</button>
						</c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${ not empty sessionScope.userID }">
							<form action="addCart.do" method="post" class="cart-addbook" onsubmit="return confirmFn();">
						...
						<c:otherwise>
							<button type="button" onclick="location.href='login.do'">로그인 후에 장바구니 추가</button>
						</c:otherwise>
					</c:choose>	
					<button type="button" onclick="location.href='bookList.do'" class="main-btn">도서목록</button>
