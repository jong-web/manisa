...
import dao.BoardDAO;
import fileProcess.FileRun;
import message.ShowMessage;
import model.Board;

@WebServlet("/JSPBookShop/boardWrite.do")
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, maxFileSize = 1024 * 1024 * 200, maxRequestSize = 1024 * 1024 * 400)
public class BoardWriteController extends HttpServlet {
	// 파일 업로드, 다운로드에 사용되는 폴더 지정
	private static final String FILES_DIR = "upload";
	// boardWrite.jsp에서 받아온 title, content, ofile, userID, pass를 boardWrite.do로 보냄
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		// 파일이 첨부되지 않았다면 fileName과 newFileName은 값은 null
		...
		if(result == 1) {
			...
			ShowMessage.alertBack(res, "글이 작성되지 않았습니다.");
		}
	}
	// 파일 이름만 추출하기
